...
var MyPluginSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    containerEl.createEl("h2", { text: "Auto Tag and Link Settings" });
      ...
      .addText((text) =>
        text
          ...
          .onChange(async (value) => {
            this.plugin.settings.tagFolderPath = value;
...
function dumpFrontmatter(frontmatter) {
  try {
    ...
    console.error("Failed to dump frontmatter:", e);
    return "";
  }
}
...
// tagProcessor.ts
async function processVaultForTags(app, tagList, file, targetFolderPath) {
  let filesToProcess = [];
  if (file) {
    ...
    if (targetFolderPath) {
      ...
      if (targetFolder instanceof import_obsidian2.TFolder) {
        await collectMarkdownFilesInFolder(targetFolder, filesToProcess);
      } else {
        new import_obsidian2.Notice(
          `Target folder "${targetFolderPath}" not found. Processing skipped.`,
          5e3
        );
        return;
      }
    } else {
      filesToProcess = app.vault.getMarkdownFiles();
    }
  }
  ...
  for (const mdFile of filesToProcess) {
    if (mdFile.path.startsWith("998_Tags")) {
      continue;
    }
    try {
      const originalContent = await app.vault.read(mdFile);
      ...
      const currentTags = Array.isArray(frontmatter.tags)
        ? frontmatter.tags
        : [];
      const tagsToAdd = /* @__PURE__ */ new Set();
      currentTags.forEach((tag) =>
        tagsToAdd.add(normalizeTagNameForCompare(tag))
      );
      let newContentWithoutFrontmatter = contentWithoutFrontmatter;
      for (const tag of tagList) {
        const regex = new RegExp(`\\b${escapeRegExp(tag)}\\b`, "gi");
        const foundMatches = newContentWithoutFrontmatter.match(regex);
        if (foundMatches && foundMatches.length > 0) {
          ...
          if (!newContentWithoutFrontmatter.match(linkRegex)) {
            newContentWithoutFrontmatter = newContentWithoutFrontmatter.replace(
              regex,
              `[[${tag}]]`
            );
          }
      ...
      frontmatter.tags = Array.from(tagsToAdd).sort();
      const newFrontmatter = dumpFrontmatter(frontmatter);
      const finalContent = `---
${newFrontmatter}---
${newContentWithoutFrontmatter.trim()}
`;
      if (originalContent !== finalContent) {
        await app.vault.modify(mdFile, finalContent);
        processedCount++;
      }
    } catch (error) {
      console.error(`Error processing file ${mdFile.path}:`, error);
      new import_obsidian2.Notice(
        `Failed to process ${mdFile.basename}. See console for details.`,
        5e3
      );
    }
  }
...
async function collectMarkdownFilesInFolder(folder, fileList) {
  for (const child of folder.children) {
    if (child instanceof import_obsidian2.TFolder) {
      await collectMarkdownFilesInFolder(child, fileList);
    } else if (
      child instanceof import_obsidian2.TFile &&
      child.extension === "md"
    ) {
      fileList.push(child);
    }
...
var DEFAULT_SETTINGS = {
  tagFolderPath: "998_Tags",
  // デフォルトのタグフォルダパス
  targetFolderPath: "",
  // デフォルトでは対象フォルダなし (Vault全体を意味する)
};
var MyPlugin = class extends import_obsidian3.Plugin {
  constructor() {
    super(...arguments);
    this.tagList = /* @__PURE__ */ new Set();
  }
  // タグリストを保持
  async onload() {
    ...
    this.registerEvent(
      this.app.vault.on("create", (file) => {
        if (
          ...
          file.path.startsWith(this.settings.tagFolderPath)
        ) {
          this.updateTagList(file, "add");
        }
      })
    );
    this.registerEvent(
      this.app.vault.on("rename", (file, oldPath) => {
        ...
        if (oldPath.startsWith(this.settings.tagFolderPath)) {
          const oldTagName = normalizeTagName(
            ((_a = oldPath.split("/").pop()) == null
              ? void 0
              : _a.replace(/\.md$/, "")) || ""
          );
          if (oldTagName) {
            this.tagList.delete(oldTagName);
          }
        }
      })
    );
    this.registerEvent(
      this.app.vault.on("delete", (file) => {
        if (
          ...
          this.updateTagList(file, "remove");
        }
      })
    );
    this.addCommand({
      ...
      editorCallback: async (editor, view) => {
        if (view.file) {
          new import_obsidian3.Notice("Processing current file...");
          await processVaultForTags(
            this.app,
            this.tagList,
            view.file,
            this.settings.targetFolderPath
          );
          new import_obsidian3.Notice("Current file processed.");
        }
      },
    });
    this.addCommand({
      ...
      callback: async () => {
        ...
        new import_obsidian3.Notice("All specified files processed.");
      },
    });
  ...
  /**
   * 指定されたフォルダからタグリストを再帰的に生成
   */
  async generateTagList() {
    this.tagList.clear();
    const tagFolder = this.app.vault.getAbstractFileByPath(
      this.settings.tagFolderPath
    );
    if (tagFolder instanceof import_obsidian3.TFolder) {
      await this.walkFolder(tagFolder);
      new import_obsidian3.Notice(`Tags loaded: ${this.tagList.size} tags.`);
    } else {
      new import_obsidian3.Notice(
  ...
  /**
   * フォルダを再帰的に走査し、ファイル名をタグとして追加
   */
  async walkFolder(folder) {
    for (const child of folder.children) {
      if (child instanceof import_obsidian3.TFolder) {
        await this.walkFolder(child);
      } else if (
        child instanceof import_obsidian3.TFile &&
        child.extension === "md"
      ) {
        const tagName = normalizeTagName(child.basename);
        if (tagName) {
   ...
   * タグリストの更新（追加/削除）
   */
  updateTagList(file, action) {
    const tagName = normalizeTagName(file.basename);
    if (tagName) {
      if (action === "add") {
        this.tagList.add(tagName);
...
*/
//# sourceMappingURL=data:application/json;base64,
